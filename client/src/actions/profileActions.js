...
export const followUser = (userId) => (dispatch) => {
  axios
    .post("http://localhost:5000/api/users/follow", { userId })
    .then((res) =>
      dispatch({
        type: FOLLOW,
...
export const unfollowUser = (userId) => (dispatch) => {
  axios
    .post("http://localhost:5000/api/users/unfollow", { userId })
    .then((res) =>
      dispatch({
        type: UNFOLLOW,
        payload: res.data.userId,
...
export const getLikedPosts = () => (dispatch) => {
  dispatch(loadPosts());
  axios
    .get(`http://localhost:5000/api/posts/liked`)
    .then((res) =>
      dispatch({
        type: GET_POSTS,
        payload: res.data,
